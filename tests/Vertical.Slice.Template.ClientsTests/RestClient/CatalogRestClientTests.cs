...
namespace Vertical.Slice.Template.ConnectedServiceClientsTests.RestClient;

public class CatalogRestClientTests(
...
) : CatalogsTestBase(sharedFixture, outputHelper)
{
    ...
    public async Task create_product_can_create_product_with_valid_data()
    {
        ...
        // Act
        var createdProductId = await CatalogsRestClient.CreateProductAsync(createProduct, CancellationToken.None);
    ...
    public async Task get_product_by_id_can_get_product_with_valid_data()
    {
        ...
        var createProduct = new AutoFaker<CreateProductClientRequestDto>()
            .RuleFor(x => x.Name, f => f.Name.FullName())
            .Generate();
        var id = await CatalogsRestClient.CreateProductAsync(createProduct, CancellationToken.None);

        // Act
        var product = await CatalogsRestClient.GetProductByIdAsync(id, CancellationToken.None);
    ...
    {
        // Act
        var productsPageList = await CatalogsRestClient.GetProductByPageAsync(
